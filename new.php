...
$sqlM="SELECT pro_url FROM projects order by pro_id desc limit 0,1";
...
$sqlA="INSERT INTO `projects` (`pro_name`, `pro_url`, `pro_backers`, `pro_daystogo`, `pro_delivery`, `pro_visible_f`, `pro_pub_id`)VALUES(NULL,'".str_replace("?ref=discovery_category_newest","/",$_POST['url'])."',NULL,NULL,NULL,NULL,NULL)";
echo $sqlA;
mysqli_query($mysqli,$sqlA);	


$sql="SELECT * FROM projects where pro_name is NULL";
...
//<a id="back-project-button" href="https://www.kickstarter.com/projects/512772051/merchants-cove/pledge/new?clicked_reward=false" class="bttn bttn-green bttn-large block mb3 keyboard-focusable" style="">Back this project</a>-
	...
	if(strpos($content, '<a class="bttn bttn-green bttn-medium" id="nav-back-project-button" title="Back this project" href="') !== false) 	
	{
		...
		$name=str_replace(array("'", "\"", "&quot;", "&#39;", "&amp;#39;", "&amp;quot;", "&#x27;", "&amp;#x27;"),"",$table1[1]);
			...
			preg_match('#backersCount&quot;:(.*),&quot;percentFunded#', $content, $USDmatch);
			//echo "--------------<p>";
				//print_r($USDmatch);
					//echo "<p>--------------<p>";
			...
			$backers=trim(substr($backers,0,$k-1));
			
			preg_match('#</span></div><span class="block dark-grey-500 type-12 type-14-md lh3-lg">(.*)</span></div><div class="ml5 ml0-lg"><div><div><span class="block type-16 type-28-md bold dark-grey-500">#', $content, $USDmatch);
			if(substr($USDmatch[1],strpos($USDmatch[1],"backer<"),7)=="backer<")($backers=1);
		...
		else
		{
			...
			preg_match('#&quot;duration&quot;:(.*),&quot;url#', $content, $table1);
			$days=$table1[1];
			if($days==""){$days=0;}			
		}
		...
		else
		{
			...
			$sql5="update projects set `pro_name`='".$name."', `pro_backers`=".$backers.", `pro_daystogo`=".$days.", `pro_delivery`='".substr($delivery,4,4)."-".$month."-01', `pro_visible_f`=1,pro_pub_id=".$row2['pub_id']." where pro_id=".$row['pro_id'].";";
			echo "<p>".$sql5."<br>";//1
			//echo " |||| <a href=".$row['pro_url'].">".$row['pro_url']."</a><br>";						
			mysqli_query($mysqli,$sql5);					
			//echo $counter."<br>";			
	...
	if(strpos($content, 'backers</b> pledged <span class="money">') !== false) 		 
	{
		...
		else
		{
			...
			$sql5="update projects set `pro_name`='".$name."', `pro_backers`=".$backers.", `pro_daystogo`=1000, `pro_delivery`='".substr($delivery,4,4)."-".$month."-01', `pro_visible_f`=1,pro_pub_id=".$row2['pub_id']." where pro_id=".$row['pro_id'].";";
			echo $sql5."<br>";//2
	...
	if(strpos($content, 'backers</b> pledged <span class="money">') === false&&strpos($content, '<a class="bttn bttn-green bttn-medium" id="nav-back-project-button" title="Back this project" href="') === false)
	{
		...
		preg_match('#<div class="ml5 ml0-lg mb2-lg"><div class="block type-16 type-24-md medium soft-black"><span>(.*)</span></div><span class="block dark-grey-500 type-12 type-14-md lh3-lg">backer#', $content, $USDmatch);
